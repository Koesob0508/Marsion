...
namespace Marsion
{
    ...
    public class MouseInputProvider : MonoBehaviour, IMouseInput
    {
        ...
        {
            Logger.LogPointer<MouseInputProvider>($"{gameObject.name} Pointer Enter");
            ((IMouseInput)this).OnPointerEnter.Invoke(eventData);
        }

        void IPointerExitHandler.OnPointerExit(PointerEventData eventData)
        {
            Logger.LogPointer<MouseInputProvider>($"{gameObject.name} Pointer Exit");
            ((IMouseInput)this).OnPointerExit.Invoke(eventData);
        }

        void IPointerDownHandler.OnPointerDown(PointerEventData eventData)
        {
            Logger.LogPointer<MouseInputProvider>($"{gameObject.name} Pointer Down");
            ((IMouseInput)this).OnPointerDown.Invoke(eventData);
        }

        void IPointerUpHandler.OnPointerUp(PointerEventData eventData)
        {
            Logger.LogPointer<MouseInputProvider>($"{gameObject.name} Pointer Up");
            ((IMouseInput)this).OnPointerUp.Invoke(eventData);
        }

        void IPointerClickHandler.OnPointerClick(PointerEventData eventData)
        {
            Logger.LogPointer<MouseInputProvider>($"{gameObject.name} Pointer Click");
            ((IMouseInput)this).OnPointerClick.Invoke(eventData);
        }

        void IBeginDragHandler.OnBeginDrag(PointerEventData eventData)
        {
            Logger.LogPointer<MouseInputProvider>($"{gameObject.name} Begin Drag");
            ((IMouseInput)this).OnBeginDrag.Invoke(eventData);
        }

        void IDragHandler.OnDrag(PointerEventData eventData)
        {
            Logger.LogPointer<MouseInputProvider>($"{gameObject.name} Drag");
            ((IMouseInput)this).OnDrag.Invoke(eventData);
        }

        void IEndDragHandler.OnEndDrag(PointerEventData eventData)
        {
            Logger.LogPointer<MouseInputProvider>($"{gameObject.name} End Drag");
            ((IMouseInput)this).OnEndDrag.Invoke(eventData);
        }

        void IDropHandler.OnDrop(PointerEventData eventData)
        {
            Logger.LogPointer<MouseInputProvider>($"{gameObject.name} Drop");
